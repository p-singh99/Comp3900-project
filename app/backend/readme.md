...
| GET        | `/users/self/history/<pagenumber>`                                  |                                       | {"history": [{xml, guid}]}, 200<br>`{“error“ :  “bad request”}`, 400 | Check user history |
...
| PUT         | `/users/self/ratings/<podcastID>`                    | `{"rating": <rating>}` |             | Update rating for podcast |
...
| GET         | `/users/self/history/<podcastID>` |   `{"rating": rating}`    | | Get listening history. With podcast set, returns listening history for a particular podcast. |
